        ...
        try {
            ...
            val result = okHttpClient.newCall(request).execute().use { response ->
                ...
                if (latest != null) {
                    ...
                    MemeOsAppDetails(packageName, appName, version, code, region, sizeBytes, date, downloadUrl)
                } else {
        ...
        try {
            ...
            val result = okHttpClient.newCall(request).execute().use { response ->
                ...
                val detailRequest = Request.Builder().url(downloadUrl)
                    .header("User-Agent", NetworkUtils.USER_AGENT).build()
                val detailResult = okHttpClient.newCall(detailRequest).execute().use { detailResponse ->
